...
import { getAllSnippets, deleteSnippet, getSnippet } from "../services/snippetService";
...
export default function FileBrowser({ isOpen, onClose, onSelect, isDark }) {
  ...
  const handleDelete = async (e, id) => {
    ...
    if (confirm(`Delete "${snippet.title || snippet.slug}"?`)) {
      setDeletingId(id);

      const { success, error } = await deleteSnippet(snippet.slug);

      if (success) {
        setSnippets(snippets.filter((s) => s.id !== id));
      } else {
      console.warn('Failed to delete snippet:', error);
      alert('Failed to delete snippet: ' + (error || 'Unknown error'));
    }
  ...
  // title ya slug se search (null-safe)
  ...
  return (
    ...
    >
      ...
      {/* Mobile search */}
      <div
        className={`px-4 py-2 md:hidden border-b ${
          isDark ? "border-gray-800" : "border-gray-200"
        }`}
      >
        <input
          ...
          <ul className="space-y-1">
            {filteredSnippets.map((snippet) => (
              <li
                key={snippet.id}
                className={`
                  flex items-start justify-between gap-3 p-3 rounded cursor-pointer
                  ${
                    isDark
                      ? "hover:bg-gray-800"
                      : "hover:bg-gray-50"
                ...
                onClick={async () => {
                  try {
                    // yahan se full snippet fetch karenge
                    const { snippet: full, error } = await getSnippet(
                      snippet.slug
                    );
                    if (error || !full) {
                      alert(
                        "Failed to load snippet: " + (error || "Not found")
                      );
                      return;
                    }
                    onSelect(full);
                    ...
                    alert("Something went wrong while loading snippet.");
                  }
                }}
              >
                ...
                >
                  <Trash2
                    size={14}
                    className={deletingId === snippet.id ? "opacity-50" : ""}
                  />
                </button>
              </li>
            ))}
